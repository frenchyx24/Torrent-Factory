  ...
  const handleGenerate = async (items: any[]) => {
    try {
      const res = await fetch('/api/generate', {
        ...
        body: JSON.stringify({ items, type: 'Série' })
      });
      if (res.ok) {
        showSuccess(`${items.length} tâche(s) lancée(s)`);
        navigate('/tasks');
      }
    } catch (e) {
        ...
        <Table>
          <TableHeader className="bg-white/5">
            <TableRow className="border-white/10">
              <TableHead className="w-12"><Checkbox className="border-white/20" onCheckedChange={(checked) => setSelected(checked ? filteredSeries.map(s => s.name) : [])}/></TableHead>
              ...
              <TableRow key={i} className="border-white/5 hover:bg-white/5">
                ...
                <TableCell>
                  <Select defaultValue="MULTI">
                    <SelectTrigger className="w-24 bg-slate-950/50 border-white/10 text-xs h-8"><SelectValue /></SelectTrigger>
