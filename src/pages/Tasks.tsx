    ...
    <Layout>
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-400 mt-1">Progression en temps réel.</p>
        </div>
      ...
      <div className="space-y-6">
        {tasks.length > 0 ? tasks.slice().reverse().map((task) => (
          <div key={task.id} className={cn(
            ...
            <div className="flex justify-between items-start mb-6">
              ...
              {['running', 'pending'].includes(task.status) && (
                <Button size="sm" variant="destructive" onClick={() => cancelTask(task.id)} className="bg-rose-600/20 text-rose-500 hover:bg-rose-600/30 border-none">
            ...
            <div className="space-y-4">
              <div className="flex justify-between text-sm">
                <span className="text-slate-300 truncate max-w-[80%]">{task.current_item_name || task.name}</span>
          ...
          <div className="text-center py-20 text-slate-500 italic flex flex-col items-center gap-4">
            <Clock className="w-12 h-12 opacity-20" /> Aucune tâche en cours.
          </div>
        )}
      </div>
    </Layout>
